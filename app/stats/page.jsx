...
export default function Home() {
  ...
  const getTotalVisits = () => {
    return Object.values(venueStats).reduce(
      (acc, venue) => acc + venue.count,
      0
    );
  };
  ...
  const hasCompletedTheme = (patrol, theme, venues) => {
    const themeVenues = venues
      .filter((venue) => venue.parentTheme === theme)
  ...
  const getThemeCompletionCount = (theme) => {
    ...
    return patrolData.filter((patrol) =>
      hasCompletedTheme(
        patrol,
        theme,
        Object.entries(venueStats).map(([id, data]) => ({
          venueId: id,
  ...
  const filterEntriesByDate = (entries, selectedDate) => {
    if (!Array.isArray(entries)) return [];
    ...
    return entries.filter((entry) => {
      const entryDate = new Date(entry.lastUpdated);
      const istDate = new Date(entryDate.getTime() + istOffset);
      return istDate.toDateString() === new Date(selectedDate).toDateString();
    });
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dateFilteredEntries = filterEntriesByDate(patrolResult, selectedDate);
        setPatrolData(dateFilteredEntries);
        setVenueStats(calculateStats(dateFilteredEntries, venueData));
      } catch (error) {
              ...
              <input
                ...
                onChange={(e) => {
                  if (isDateValid(e.target.value)) {
                    ...
                    toast.error("Please select a date between Jan 28 and Feb 3, 2025");
                  }
                }}
                min="2025-01-28"
                max="2025-02-03"
            ...
            <div className="p-4 border-b border-gray-200">
              <h2 className="text-xl font-bold text-gray-800">
                ...
                <span className="ml-2 text-sm font-normal text-gray-500">
                  ({getThemeCompletionCount(selectedTheme)} patrols completed
                  all venues)
                </span>
              </h2>
            </div>
